...
function loadTowerInfo(index){
    ...
    for (let i = 0; i < 3; i++) {
        ...
        let text = upgrades[index][i].name
        if(checkIfUpgradeUnlockable(index, i) == 1){
            unlockedClass = "unlockable"
            text = formatNumberToString(upgrades[index][i].xpToUnlock) + " XP"
        }
        else if(checkIfUpgradeUnlockable(index, i) == -1){
            unlockedClass = "notUnlockable"
            text = formatNumberToString(upgrades[index][i].xpToUnlock) + " XP"
        }
        string += `<div class="upgrade ${unlockedClass}" onclick="loadUpgradeinfo(${index},${i});selectUpgrade(${index},${i})"><p>${text}</p></div>`
        if(i < 2){
