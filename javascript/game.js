...
let explosion_container = document.getElementById('explosion-container')
        ...
        // position chosen
            ...
            if(checkIfSpotIsValid()){
                ...
                if(cash - towerInfo[i].cost >= 0){
                    ...
                    let newTowerInfo = structuredClone(towerInfo[i])

                    upgradeBox_container.innerHTML += `
                    <div class="upgradeBox" id="upgradeBox${currentTowerIndex}">
                        <div class="x-button x" onclick="showRangeAndUpgradeBox(${currentTowerIndex})"></div>
...
function checkIfSpotIsValid(){
    if(selectedTower.index != -1){
        showRangeAndUpgradeBox(selectedTower.index)
    }
    
    //checkIfPositionInterferesWithShop
    if(isColliding(tower, towerShopContainer)) return false;
